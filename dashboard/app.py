...
Supports interactive exploration and visualization for:
1. 2D Developing Laminar Pipe Flow: (r/R, x/L, Re) -> (u, v)
2. Legacy 1D Fully Developed Hagen-Poiseuille Flow: (r, u_max) -> u
...
# -----------------------------------------------------------------------------
# Legacy 1D Model Construction Helper
# -----------------------------------------------------------------------------
class Legacy1DMLP(nn.Module):
    """Fallback module to load legacy 1D MLP checkpoints (2 inputs -> 1 output)."""

    def __init__(self, depth: int, width: int, activation_name: str, hard_bc: bool = False, R: float = 1.0) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.hard_bc:
            u_max = x[:, 1:2]
            return (1.0 - (r / self.R) ** 2) * (u_max + r * (self.R - r) * u_raw)
        return u_raw
...
def get_available_runs(results_dir: str = RESULTS_DIR) -> list[str]:
    ...
    For experiments with the same base name, keeps only the most recent run.
    """
...
def load_trained_model(run_name: str, results_dir: str = RESULTS_DIR):
    ...
    Determines whether the run is 2D developing pipe flow or legacy 1D flow.

    Returns:
        Tuple of (model, config, is_2d_developing).
    """
    ...
    # Detect dimensionality from the input layer weights
    first_weight_key = next((k for k in state_dict.keys() if k.endswith("0.weight") or k == "net.0.weight"), None)
...
selected_run = st.sidebar.selectbox(
    ...
    help="Select an experiment directory containing model.pt and config.yaml.",
)
...
# =============================================================================
# MODE 1: 2D Developing Pipe Flow
# =============================================================================
if is_2d:
    st.caption(
        "**Formulation**: 2D developing pipe flow. "
        "Inputs: `(r/R, x/L, Re)` → Outputs: `(u, v)`. "
        "Physics: boundary layer axial momentum and continuity."
    )
    ...
    Re_val = st.sidebar.slider(
        ...
        step=10.0,
        help="Operating Reynolds number based on pipe diameter.",
    )
    ...
    x_val = st.sidebar.slider(
        ...
        value=float(L * 0.25),
        step=0.1,
        help="Axial position along the pipe length.",
    )

    # Physical quantities
    D = 2.0 * R
    u_in = (Re_val * nu) / D
    # Hydrodynamic entrance length: Le ≈ 0.05 * Re * D
    ...
    # 1D radial profile at selected x_val
    n_r_line = 200
    ...
    # Analytical asymptotic Hagen-Poiseuille curve
    hp_exact = 2.0 * u_in * (1.0 - (r_line / R) ** 2)

    # Centerline velocity evolution along x
    n_x_line = 200
    ...
    # Plot 1: 2D Velocity Field Heatmap
    # Mirrored across r=0 to display full pipe diameter [-R, +R]
    ...
    # Add vertical indicator for current x
    fig_heatmap.add_vline(
        x=x_val,
        line_width=2.5,
        line_dash="dash",
        line_color="#ffffff",
        annotation_text=f"Selected station x = {x_val:.1f} m",
        annotation_position="top right",
    )
    # Add vertical indicator for entrance length
    if entry_length <= L:
        fig_heatmap.add_vline(
            x=entry_length,
            line_width=2,
    ...
    fig_heatmap.update_layout(
        title=dict(
            text=f"2D Axial Velocity Field u(r, x) at Re = {Re_val:.0f} (Full pipe diameter)",
            font=dict(size=16),
        ),
    ...
    with c1:
        ...
        fig_prof.update_layout(
            title=dict(
                text=f"Axial Velocity Profile u(r) at x = {x_val:.1f} m (Re = {Re_val:.0f})",
                font=dict(size=14),
            ),
    ...
    with c2:
        ...
        fig_center.update_layout(
            title=dict(
                text=f"Centerline Axial Velocity Evolution u(0, x) along pipe",
                font=dict(size=14),
            ),
            xaxis_title="Axial position x [m]",
    ...
    with c3:
        ...
        fig_multi.update_layout(
            title=dict(
                text=f"Profile Development Across Axial Stations (Re = {Re_val:.0f})",
                font=dict(size=14),
            ),
            xaxis_title="Radial position r [m]",
    ...
    with c4:
        ...
        fig_v.update_layout(
            title=dict(
                text=f"Radial Velocity Component v(r) at x = {x_val:.1f} m",
                font=dict(size=14),
            ),
            xaxis_title="Radial position r [m]",
...
# =============================================================================
# MODE 2: Legacy 1D Hagen-Poiseuille Flow
# =============================================================================
else:
    st.info(
        "ℹ️ **Legacy 1D Model**: The selected run belongs to the 1D Hagen-Poiseuille experiments (exp_001–exp_028). "
        "The current codebase has transitioned to 2D developing pipe flow (exp_029+)."
    )
    ...
    with st.container(border=True):
        ...
        m1.metric("L2 error", f"{l2_error:.4e}")
        m2.metric("Max absolute error", f"{max_error:.4e}")
        m3.metric("Relative L2 error", f"{relative_l2_error:.4e}")
    ...
    with c1:
        fig1 = go.Figure()
        fig1.add_trace(go.Scatter(x=r_np, y=u_pred_np, mode="lines", name="PINN prediction", line=dict(color="#1f77b4", width=3)))
        fig1.add_trace(go.Scatter(x=r_np, y=u_exact_np, mode="lines", name="Analytical exact", line=dict(color="#ff7f0e", width=2.5, dash="dash")))
        fig1.update_layout(
            title=dict(text=f"Velocity profile (u_max = {u_max_val:.1f}, R = {pipe_radius_R:.2f})", font=dict(size=14)),
    ...
    with c2:
        ...
        fig2.update_layout(
            title=dict(text=f"Pointwise absolute error (u_max = {u_max_val:.1f})", font=dict(size=14)),
            xaxis_title="Radial position r",
